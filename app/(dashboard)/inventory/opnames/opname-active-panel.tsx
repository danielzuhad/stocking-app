...
import type { ActiveStockOpnameType } from '@/types';
...
/** Stock opname action workspace (start/update/finalize/void). */
export function OpnameActivePanel({
  can_write,
  active_stock_opname,
}: {
  can_write: boolean;
    ...
    <Card>
      <CardHeader className="flex flex-row items-center justify-between gap-2">
        <CardTitle className="text-base">
          {active_stock_opname ? 'Stok Opname Aktif' : 'Mulai Stok Opname'}
        </CardTitle>
        {active_stock_opname ? (
          ...
          can_write ? (
            <div className="space-y-3">
              <div className="space-y-1">
                <label htmlFor="start-opname-note" className="text-sm font-medium">
                  Catatan (opsional)
                </label>
                <Textarea
                  id="start-opname-note"
                  rows={2}
                  value={startOpnameNote}
                  onChange={(event) => setStartOpnameNote(event.currentTarget.value)}
            ...
            <div className="text-muted-foreground text-sm">
              Anda tidak memiliki akses untuk memulai stok opname.
            </div>
          )
        ) : (
